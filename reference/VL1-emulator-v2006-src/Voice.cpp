...
{ 
	...
	m_outBufferSize = (kMinBLEPSize/kMinBLEPOversampling)*(gSampleRate/kMinBLEPSampleRate);
...
void CVoice::Reset()
{
	...
	m_lfoVibrato.Reset();
	m_lfoTremolo.Reset();
	m_lpf.SetCutoff(0.95f*gSampleRate/gOversampling,gSampleRate);
	m_lpf.SetQ(4.0f);
	m_lpf2.SetCutoff(0.45f*gSampleRate,gSampleRate);
	m_lpf2.SetQ(2.0f);
	//m_lpf.SetChebychef1();
...
{ 
	...
	{
		case kSound: 
			if (value<0.1f) m_pWave = &gWavePiano; // 0
			else if (value<0.2f) m_pWave = &gWaveFantasy; // 1
			else if (value<0.3f) m_pWave = &gWaveViolin; // 2
			else if (value<0.4f) m_pWave = &gWaveFlute; // 3
			else if (value<0.5f) m_pWave = &gWaveGuitar1; // 4
			else if (value<0.6f) m_pWave = &gWaveGuitar2; // 5
			else if (value<0.7f) m_pWave = &gWaveEnglishHorn; // 6
			else if (value<0.8f) m_pWave = &gWavePiano; // 7, with modulation = electro 1
			else if (value<0.9f) m_pWave = &gWaveFantasy; // 8, with modulation = electro 2
			else m_pWave = &gWaveViolin; // 9, with modulation = electro 3
	...
	//m_antialias.SetCutoff(gSampleRate,gSampleRate*gOversampling);
}
...
void CVoice::NoteOn(float frequency, float velocity)
{
	...
	m_phaseInc = m_pWave->GetPitchScale()*m_tune*octave*frequency*((float)m_pWave->GetSize()/(gSampleRate*gOversampling));
	m_phase = 0.0f;
	m_velocity = velocity;
	m_vca.Gate(TRUE);
...
float CVoice::Clock()
{
	...
	sample *= m_velocity/gOversampling;
	//sample *= m_velocity;
