...
void CLineInTime::Setup(float sampleRate, int oversampling)
{
...
void CLineInTime::Initialize(float t0, float y0, float t1, float y1, bool bReset)
{
	...
	m_s = 0; // In samples.
	m_sStep = 0.001f*m_tRes*m_sampleRate*m_oversampling; // In samples.

	m_y0 = Minf(y0,y1);
	m_y1 = Maxf(y0,y1);
	m_dy = (m_y1-m_y0)/(0.001f*(m_t1-m_t0)*m_sampleRate*m_oversampling);
...
float CLineInTime::Clock()
{
	...
	if (m_bUp)
	{
		if (m_y<m_y1)
		{
			...
			{
				m_sStep = 0.001f*m_tRes*m_sampleRate*m_oversampling;
